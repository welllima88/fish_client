...
//	m_fontNormal.CreateFont(12, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, 
//		OUT_DEFAULT_PRECIS, CLIP_CHARACTER_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH|FF_SWISS, "±¼¸²");

	m_bToggleEvent = FALSE;
	m_pParentWnd = NULL;
...
BEGIN_MESSAGE_MAP(CACEdit, CEdit)
	//{{AFX_MSG_MAP(CACEdit)
...
LRESULT CACEdit::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch(message) 
	{
	case WM_IME_NOTIFY:
		{
			if( m_bToggleEvent == TRUE )
			{
				::SendMessage(m_pParentWnd->GetSafeHwnd(), UM_COMPLETECHAR, NULL, NULL);
				m_bToggleEvent = FALSE;
			}					
