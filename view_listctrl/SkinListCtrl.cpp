...
void CSkinListCtrl::OnPaint() 
{
	...
	memDC.FillSolidRect(clip, GetBkColor());
...
void CSkinListCtrl::PositionScrollBars()
{
	...
	if(pParent->GetStyle() & WS_CAPTION)    nTitleBarHeight = GetSystemMetrics(SM_CYSIZE);
    ...
    windowRect.InflateRect(nDialogFrameWidth, nTitleBarHeight+nDialogFrameHeight, nDialogFrameWidth, nTitleBarHeight+nDialogFrameHeight);
