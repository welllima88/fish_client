...
BEGIN_MESSAGE_MAP(CFishButton, CButton)
    //{{AFX_MSG_MAP(CFishButton)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CFishButton::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CFishButton::PreSubclassWindow() 
{
	UINT nBS;
...
BOOL CFishButton::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
HBRUSH CFishButton::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void CFishButton::OnSysColorChange() 
{
...
LRESULT CFishButton::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CFishButton::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CFishButton::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck
...
#ifdef	BTNST_USE_BCMENU
LRESULT CFishButton::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
	LRESULT lResult;
...
#ifdef	BTNST_USE_BCMENU
void CFishButton::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	BOOL bSetFlag = FALSE;
...
void CFishButton::OnEnable(BOOL bEnable) 
{
...
void CFishButton::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CFishButton::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CFishButton::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CFishButton::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CFishButton::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CFishButton::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CFishButton::OnClicked() 
{	
	SetFocus();
...
void CFishButton::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
...
void CFishButton::PaintBk(CDC* pDC)
{
...
HBITMAP CFishButton::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CFishButton::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CFishButton::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CFishButton::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
void CFishButton::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
...
//
HBITMAP CFishButton::CreateGrayscaleBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
...
//		If the function fails, the return value is NULL.
//
HBITMAP CFishButton::CreateDarkerBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
	HBITMAP		hGrayBitmap = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	HBITMAP		hOldBmp1 = NULL, hOldBmp2 = NULL;
...
//
HICON CFishButton::CreateGrayscaleIcon(HICON hIcon)
{
...
//		If the function fails, the return value is NULL.
//
HICON CFishButton::CreateDarkerIcon(HICON hIcon)
{
	HICON		hGrayIcon = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	BITMAP		bmp;
...
COLORREF CFishButton::DarkenColor(COLORREF crColor, double dFactor)
{
...
//
DWORD CFishButton::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
//			Failed loading the specified resource.
//
DWORD CFishButton::SetIcon(int nIconIn, int nIconOut)
{
...
//			Failed loading the specified resource.
//
DWORD CFishButton::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CFishButton::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//			Failed creating mask bitmap.
//
DWORD CFishButton::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
...
//				By default, CFishButton buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CFishButton::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CFishButton buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CFishButton::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//				By default, CFishButton buttons have BTNST_PRESSED_LEFTRIGHT style.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CFishButton::SetPressedStyle(BYTE byStyle, BOOL bRepaint)
{
	switch (byStyle)
	{
...
//			Function executed successfully.
//
DWORD CFishButton::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CFishButton::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CFishButton::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CFishButton::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CFishButton::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CFishButton::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CFishButton::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CFishButton::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CFishButton::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//
void CFishButton::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CFishButton::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CFishButton::ActivateTooltip(BOOL bActivate)
{
...
//			Function executed successfully.
//
DWORD CFishButton::EnableBalloonTooltip()
{
...
//				If TRUE the control will be repainted.
//
void CFishButton::DrawTransparent(BOOL bRepaint)
{
...
DWORD CFishButton::SetBk(CDC* pDC)
{
...
//			Function executed successfully.
//
DWORD CFishButton::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CFishButton::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CFishButton::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
...
//			Function executed successfully.
//
DWORD CFishButton::SetMenuCallback(HWND hWnd, UINT nMessage, LPARAM lParam)
{
...
//
void CFishButton::SizeToContent()
{
	if (m_csIcons[0].hIcon)
	{
		m_ptImageOrg.x = 0;
...
//
#ifdef	BTNST_USE_SOUND
DWORD CFishButton::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
...
// This is a virtual function that can be rewritten in CFishButton-derived classes
...
//			Function executed successfully.
//
DWORD CFishButton::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CFishButton::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	// Draw pressed button
	if (m_bIsPressed)
	{
