...
#include "stdafx.h"
#include <windowsx.h>
#include "MultipleSelectionTree.h"
#include ".\multipleselectiontree.h"
...
/////////////////////////////////////////////////////////////////////////////
// CMultiSelectableTreeCtrl

IMPLEMENT_DYNAMIC(CMultiSelectableTreeCtrl, CTreeCtrl)

CMultiSelectableTreeCtrl::CMultiSelectableTreeCtrl()
{
	m_bMulti = TRUE;
	m_hSelect = NULL;
	m_bBandLabel = FALSE;
...
#define CTreeCtrl	CTreeCtrl
BEGIN_MESSAGE_MAP(CMultiSelectableTreeCtrl, CTreeCtrl)
#undef CTreeCtrl
	//{{AFX_MSG_MAP(CMultiSelectableTreeCtrl)
	ON_WM_LBUTTONDOWN()
	ON_WM_SETFOCUS()
	ON_WM_KILLFOCUS()
	ON_WM_RBUTTONDOWN()
	ON_NOTIFY_REFLECT_EX(TVN_ITEMEXPANDING, OhTreeItemExpanding)
	ON_WM_KEYDOWN()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CMultiSelectableTreeCtrl message handlers

/////////////////////////////////////////////////////////////////////////////
// GetSelectedCount
// - returns number of selection tree items

UINT CMultiSelectableTreeCtrl::GetSelectedCount() const
{
    UINT nCount = 0;
...
BOOL CMultiSelectableTreeCtrl::SetMultiSelect(BOOL bMulti)
{
	BOOL b = m_bMulti;
	m_bMulti = bMulti;
	if (!m_bMulti) {
...
BOOL CMultiSelectableTreeCtrl::SetItemState(HTREEITEM hItem, UINT nState, UINT nStateMask)
{
	ASSERT(hItem);
...
UINT CMultiSelectableTreeCtrl::GetItemState(HTREEITEM hItem, UINT nStateMask) const
{
...
BOOL CMultiSelectableTreeCtrl::SelectItem(HTREEITEM hItem)
{
	if (m_bMulti) {
...
BOOL CMultiSelectableTreeCtrl::FocusItem(HTREEITEM hItem)
{
	ASSERT(m_bMulti);
...
BOOL CMultiSelectableTreeCtrl::_SendNotify(LPNMHDR pNMHDR)
{
...
HTREEITEM CMultiSelectableTreeCtrl::GetFirstSelectedItem() const
{
...
HTREEITEM CMultiSelectableTreeCtrl::GetNextSelectedItem(HTREEITEM hItem) const
{
    if(hItem)
...
void CMultiSelectableTreeCtrl::SelectAll(BOOL bSelect /*= TRUE*/)
{
	bSelect = !!bSelect;	//ensure 0 or 1
	UINT nState = bSelect ? TVIS_SELECTED : ~TVIS_SELECTED;
	HTREEITEM hItem = GetRootItem();
	while (hItem) {
...
void CMultiSelectableTreeCtrl::SelectAllIgnore(BOOL bSelect, HTREEITEM hIgnore)
{
...
void CMultiSelectableTreeCtrl::SelectRange(HTREEITEM hFirst, HTREEITEM hLast, BOOL bOnly /*= TRUE*/)
{
...
void CMultiSelectableTreeCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	OnButtonDown(TRUE, nFlags, point);
//    CTreeCtrl::OnLButtonDown(nFlags, point);
}

void CMultiSelectableTreeCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	OnButtonDown(FALSE, nFlags, point);
//    CTreeCtrl::OnLButtonDown(nFlags, point);
}

void CMultiSelectableTreeCtrl::OnButtonDown(BOOL bLeft, UINT nFlags, CPoint point)
{
	UINT nHF;
	HTREEITEM hItem;
...
void CMultiSelectableTreeCtrl::DoPreSelection(HTREEITEM hItem, BOOL bLeft, UINT nFlags)
{
	if (bLeft) {
...
void CMultiSelectableTreeCtrl::DoAction(HTREEITEM hItem, BOOL bLeft, UINT nFlags, CPoint point)
{
	::SetCapture(m_hWnd);
...
void CMultiSelectableTreeCtrl::DoBanding(BOOL bLeft, UINT nFlags, CPoint point)
{
	if (::GetFocus() != m_hWnd)
		::SetFocus(m_hWnd);
...
void CMultiSelectableTreeCtrl::UpdateSelectionForRect(LPCRECT pRect, UINT nFlags, CTreeItemList& listITEM)
{
	CRect rect;
	BOOL bSel;
	POSITION pos;
	...
	while (hItem) {
		...
		if (rect.IntersectRect(rect, pRect)) {
			//item in rect
			pos = listITEM.Find(hItem);
			if (!bSel && !pos)
				SetItemState(hItem, TVIS_SELECTED, TVIS_SELECTED);
			else if (_bCtrl && pos)
				SetItemState(hItem, 0, TVIS_SELECTED);
			else if (_bShift && pos)
				listITEM.RemoveAt(pos);		//if shift and in rect, don't lock anymore
		}
		else {
			//item not in rect
			pos = listITEM.Find(hItem);
...
void CMultiSelectableTreeCtrl::OnSetFocus(CWnd* pOldWnd) 
{
...
void CMultiSelectableTreeCtrl::OnKillFocus(CWnd* pNewWnd) 
{
...
BOOL CMultiSelectableTreeCtrl::OhTreeItemExpanding(NMHDR* pNMHDR, LRESULT* pResult) 
{
	if (!m_bMulti)
		return FALSE;
...
BOOL CMultiSelectableTreeCtrl::SelectChildren(HTREEITEM hParent, BOOL bSelect /*= TRUE*/, BOOL bRecurse /*= TRUE*/)
{
...
void CMultiSelectableTreeCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if (!m_bMulti) {
...
void CMultiSelectableTreeCtrl::GetSelectedList(CTreeItemList& listITEM) const
{
	listITEM.RemoveAll();

	HTREEITEM hItem = GetFirstSelectedItem();
	while (hItem) {
		listITEM.AddTail(hItem);
		hItem = GetNextSelectedItem(hItem);
	}
}